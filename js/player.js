...
// 处理数值更新
function updateSkipDuration (event, type) {
    const numValue = parseInt(event.target.value)
    if (!isNaN(numValue) && numValue >= 0) {
        if (type === 'intro') {
            skipIntroDuration = numValue
            localStorage.setItem('skipIntroDuration', numValue)
        } else if (type === 'ending') {
            skipEndingDuration = numValue
            localStorage.setItem('skipEndingDuration', numValue)
        }
        // 强制更新设置菜单显示
        art.setting.update()
    }
...
function initPlayer (videoUrl) {
        ...
        settings: [
            ...
            // 片头时长设置
            {
                ...
                tooltip: `<input type="text" 
                            value="${skipIntroDuration}" 
                            inputmode="numeric"
                            style="width:60px;text-align:center;background:#333;color:#fff;border:1px solid #666;border-radius:4px;margin:0;padding:2px 5px;outline:none;"
                            onchange="updateSkipDuration(event, 'intro')"
                        >` + '秒',
            },
            // 片尾时长设置
            {
                html: '片尾时长(秒)',
                tooltip: `<input type="text" 
                            value="${skipEndingDuration}"  
                            inputmode="numeric"
                            style="width:60px;text-align:center;background:#333;color:#fff;border:1px solid #666;border-radius:4px;margin:0;padding:2px 5px;outline:none;"
                            onchange="updateSkipDuration(event, 'ending')"
                        >` + '秒',
            }
        ],
        customType: {
    ...
    art.on('ready', () => {
        ...
        skipEndingButton = art.controls.add({
            ...
            style: {
                display: 'none',  // 初始隐藏
                opacity: '0.8',   // 透明度与其他控件一致
                padding: '0 8px', // 调整内边距
                cursor: 'pointer',
                transition: 'opacity 0.2s ease',
                height: '100%',   // 确保高度与其他控件一致
                display: 'flex',  // 使用flex布局
                alignItems: 'center', // 垂直居中
            },
            click: function () {
                art.currentTime = art.duration - 1 // 直接跳到结束，触发 ended 事件
            }
        })

        // 添加hover效果
        if (skipEndingButton && skipEndingButton.$el) {
            skipEndingButton.$el.addEventListener('mouseover', () => {
                skipEndingButton.$el.style.opacity = '1';
            });
            skipEndingButton.$el.addEventListener('mouseout', () => {
                skipEndingButton.$el.style.opacity = '0.8';
            });
        }
    ...
    art.on('video:timeupdate', function () {
        ...
        if (skipEnabled && currentTime < skipIntroDuration) {
            ...
            //     showShortcutHint(`自动跳过片头 (${Math.floor(skipIntroDuration - currentTime)}秒)`)
            // }
        ...
        if (skipEnabled && duration > 0 && autoplayEnabled &&
            ...
            art.currentTime = duration - 1 // 直接跳到结束，触发 ended 事件
            return
        }
